...
本文件是随机专注计时器 3.0 的实现合同与当前行为说明。它统一以下需求：
...
- 实现可选的“仍在任务/刚才走神/正在心流”反馈与当前周期内的自适应调频。
...
| 强提醒焦点 | 覆盖完整多显示器虚拟桌面并主动获取焦点；平衡提醒不强抢其他应用焦点；两者都保留跳过入口 |
...
默认不显示已经抽取的准确提醒时刻，以保持不可预测性并减少等待焦虑。用户可主动显示只读剩余时间；阶段边界先到时必须提示“阶段切换后重新抽取”，不能把边界误报成提醒。

启用自适应反馈后，提醒窗口提供“仍在任务”“刚才走神”“心流中，延后”三个动作，运行页显示当前周期计数。连续 2 次走神使下一次基础抽样乘 0.75，连续 3 次仍在任务乘 1.25，心流延后乘 1.5；跳过、超时或未达到阈值使用 1.0。倍率只消费一次，连续计数保留；反馈混合时重置相反方向的连续计数。阶段边界与会话截止仍优先。反馈只存在当前 `FocusSession` 内，不持久化、不联网。

可选悬浮计时条只显示剩余时间和当前阶段，复用主会话的显示刷新，不创建独立计时器。窗口不进入任务栏、可拖动，拖动释放时保存坐标；恢复时按坐标选择最近显示器工作区并夹回可见范围，因此支持负坐标显示器和显示器拔除后的恢复。右键或 × 只隐藏当前周期；停止、进入大休息或退出时销毁。
...
同一 schema 中缺失的嵌套字段按默认值递归补全，不得因此丢弃其他用户设置；无法解析或校验失败的配置先改名保留为 `settings.invalid.json`，再使用默认设置启动。正式构建前暂存安装目录中的配置和日志，构建结束后恢复。
...
### 9.1 参考项目调研（2026-07-15）

- `JulesSu111/Random-Focus-Timer`（审查提交 `b369bf8`）提供“默认遮蔽下一次响铃时间”的产品思路；审查时未发现许可证，因此不复制其源码或资源，只独立实现该交互。
- `kaka1025/my-time-logger`（审查提交 `2e9deec`，`docs/LICENSE` 为 MIT）提供配置缺字段自动补全、全局快捷键、悬浮显示和学习记录等思路。已独立实现配置兼容、原生快捷键和轻量悬浮状态；统计与奖励系统会扩大产品边界，暂不引入。
...
- 全局快捷键默认使用 `Alt+Shift+P` 暂停/继续、`Alt+Shift+O` 显示/隐藏主界面；允许 `Ctrl/Alt/Shift/Win` 加 `A–Z`、`0–9`、`F1–F12` 或 `Space`，禁止裸键和两个动作使用同一组合；使用 `MOD_NOREPEAT`，解析错误或注册冲突时在接触 Win32 前拒绝或完整回滚并关闭设置；
- 开机启动使用当前用户的 Windows Run 启动项，不要求管理员权限；可选择正常显示主界面，或带 `--startup` 参数在 Tk 主循环前静默到托盘；托盘初始化失败时保留主窗口；
- “退出程序”在活动周期中要求确认；
- 统一退出顺序：标记 `SHUTTING_DOWN`、使 `session_id` 失效、取消回调、关闭提醒和悬浮状态、停止音频、释放全局快捷键、停止托盘、销毁 Tk 根窗口；
...
- 手动暂停保持下一提醒剩余等待；
- 在提醒已经显示时暂停会关闭提醒；恢复后重新安全随机抽取，且不会误播回归铃；
- 睡眠恢复丢弃旧提醒并等待确认；
- GUI 回调延迟不会造成显示漂移；
- 停止后快速重启不会执行旧会话回调；
