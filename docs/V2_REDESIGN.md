...
可选悬浮计时条只显示剩余时间和当前阶段，复用主会话的显示刷新，不创建独立计时器。窗口不进入任务栏、可拖动，右键或 × 只隐藏当前周期；停止、进入大休息或退出时销毁。
...
- 全局快捷键使用 `Alt+Shift+P` 暂停/继续、`Alt+Shift+O` 显示/隐藏主界面；使用 `MOD_NOREPEAT`，注册冲突时完整回滚并关闭设置；
